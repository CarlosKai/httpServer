...
{
    //http_conn类对象
    users = new http_conn[MAX_FD];  //支持最大用户连接数
                     ...
                     int opt_linger, int trigmode, int sql_num, int thread_num, int close_log, int actor_model)
{
    ...
    m_OPT_LINGER = opt_linger;   //优雅关闭链接，默认不使用
    m_TRIGMode = trigmode;
    m_close_log = close_log;    //是否关闭日志
    m_actormodel = actor_model; //默认proactor
}
...
void WebServer::eventListen()
{
    ...
    //优雅关闭连接
    //如果m_OPT_LINGER为0，则立即关闭连接，不等待未发送的数据。
    if (0 == m_OPT_LINGER)
    {
    ...
    {
        //如果m_OPT_LINGER为1，则等待未发送的数据发送完毕后再关闭连接。
